...
{
 var ret = true;
 var val = objValue.replace(/^\s+|\s+$/g,"");//trim
...
var ret = true;
	  ...
	  if(eval(objValue) >=  eval(strLessThan)) 
	  { 
	    if(!strError || strError.length ==0) 
	    { 
...
var ret = true;
     ...
     if(eval(objValue) <=  eval(strGreaterThan)) 
      { 
        if(!strError || strError.length ==0) 
        { 
